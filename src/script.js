class CardStack {
  constructor() {
    ...
    this.visibleContainer = document.querySelector("#visible-cards-container");
    this.activeIndex = 0;
    this.globalScrollProgress = 0;
    this.cardCount = document.querySelectorAll(".scrollable-card").length;
    this.visibleCards = [];
    this.shuffleCards(); // Shuffle before initialization
    this.init();
  }
  ...
  // Shuffle both scrollable and visible cards while maintaining their relationship
  shuffleCards() {
    // Get arrays of both types of cards
    const scrollableCards = Array.from(this.scrollableContainer.getElementsByClassName('scrollable-card'));
    const visibleCards = Array.from(this.visibleContainer.getElementsByClassName('visible-card'));

    // Generate random order (array of indices)
    const indices = Array.from({ length: scrollableCards.length }, (_, i) => i);
    this.shuffleArray(indices);

    // Reorder cards according to shuffled indices
    indices.forEach((newIndex, currentIndex) => {
      this.scrollableContainer.appendChild(scrollableCards[newIndex]);
      this.visibleContainer.appendChild(visibleCards[newIndex]);
    });
  }
