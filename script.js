...
class VisibleCard {
  ...
  applyStyles() {
    this.element.style.transform = `translateX(${this.translateX - 50
      }%) translateY(-50%) translateZ(${this.translateZ}px) rotateY(${this.rotateY
...
/*
 Shuffle the order of visible and scrollable cards so the layout is random on each load.
 The algorithm will reorder both `#visible-cards-container` and `#scrollable-container`
 using the same randomized index order so the visual stack and the scroll pages remain
 aligned.
*/
function shuffleCardsOnLoad() {
  const visibleContainer = document.querySelector('#visible-cards-container');
  const scrollableContainer = document.querySelector('#scrollable-container');

  if (!visibleContainer || !scrollableContainer) return;

  const visible = Array.from(visibleContainer.querySelectorAll('.visible-card'));
  const scrollable = Array.from(scrollableContainer.querySelectorAll('.scrollable-card'));

  if (visible.length !== scrollable.length) {
    console.warn('shuffleCardsOnLoad: different number of visible and scrollable cards; shuffling visible only.');
    // shuffle visible only
    for (let i = visible.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      visibleContainer.appendChild(visible[j]);
      visible.splice(j, 1);
    }
    return;
  }
  ...
  // append in shuffled order to both containers
  indices.forEach((idx) => {
    visibleContainer.appendChild(visible[idx]);
    scrollableContainer.appendChild(scrollable[idx]);
  });

  // ensure scroller starts at the first page
  scrollableContainer.scrollLeft = 0;
}

// Shuffle before creating the CardStack so indices and counts match
shuffleCardsOnLoad();
